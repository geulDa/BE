...
public interface UserPostCardRepository extends JpaRepository<UserPostCard, Long> {
    ...
    List<UserPostCard> findByMemberIdWithPostCard(@Param("memberId") Long memberId);

    @Query("SELECT upc FROM UserPostCard upc " +
           "JOIN FETCH upc.postcard " +
           "WHERE upc.member.id = :memberId AND upc.place.id = :placeId")
    Optional<UserPostCard> findByMemberIdAndPlaceIdWithDetails(@Param("memberId") Long memberId, @Param("placeId") Long placeId);
}
